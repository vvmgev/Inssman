...
import { EditorLanguage, HeaderModificationType, QueryParamAction } from 'src/models/formFieldModel';
...
function FormBuilder({
  ruleMetaData, onChange, error, pageType,
}) {
  ...
  const editorLangOptions = useMemo(() => Object.entries(EditorLanguage).reduce((previous, [value, label]) => {
  ...
  const onAddHeader = (field) => {
    onChange({
      target: {
        name: 'headers',
        value: [...ruleMetaData.headers, {
          header: '', operation: 'set', value: '', type: HeaderModificationType.REQUEST,
        }],
      },
    }, field);
  };
