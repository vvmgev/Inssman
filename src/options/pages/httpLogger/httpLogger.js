...
function HTTPLogger({ clientName, showOpenWindowBtn = true }) {
  ...
  const onMessage = (message) => {
    ...
    const {
      requestId, requestHeaders, responseHeaders, method, url, type, ip, fromCache, statusCode,
      ...
      <ColorCover classes="h-[50%] pb-0">
        ...
        <ul>
          {Object.entries(requestList)
            .filter(([_, request]) => request.url.includes(search))
          ...
          <>
            ...
            <ul>
              {requestList[activeReuquestId]?.requestHeaders?.map(({ name, value }, index) => (
                <div key={index} className="gap-2 text-sm overflow-y-auto border-b border-slate-700 w-full flex whitespace-nowrap py-1 hover:bg-slate-800 hover:bg-opacity-40">
