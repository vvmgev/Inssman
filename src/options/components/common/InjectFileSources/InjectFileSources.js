...
function InjectFileSources({ onChange, ruleMetaData, error }) {
  const {
    editorLang, editorValue, fileSourceType = InjectFileSource.CODE, tagSelector, tagSelectorOperator, fileSource,
  } = ruleMetaData;

  const editorLangOptions = useMemo(() => Object.entries(InjectFileType).reduce((previous, [value, label]) => {
    previous.push({ value: value.toLowerCase(), label });
    return previous;
  }, []), []);

  const injectFileSourceOptions = useMemo(() => Object.entries(InjectFileSource).reduce((previous, [value, label]) => {
    previous.push({ value: value.toLowerCase(), label });
    return previous;
  }, []), []);

  const injectFileOperatorOptions = useMemo(() => Object.entries(InjectFileOperator).reduce((previous, [value, label]) => {
    previous.push({ value: value.toLowerCase(), label });
    return previous;
  }, []), []);
