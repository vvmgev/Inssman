...
import { IconsMap, PageName } from 'src/models/formFieldModel';
...
function RuleList({ search = '', fullColumns = true, listClasses = '' }) {
  const [data, setData] = useState([]);
  const duplicateRule = (id) => chrome.runtime.sendMessage({ action: PostMessageAction.CopyRuleById, data: { id } }, () => getData());
  const cutString = (string) => (string.length > COUNT_SYMBOLS ? `${string.slice(0, COUNT_SYMBOLS)}...` : string);
  const getData = () => chrome.runtime.sendMessage({ action: PostMessageAction.GetStorageRules }, setData);
  const onChangeRuleStatus = (event, id) => chrome.runtime.sendMessage({ action: PostMessageAction.ChangeRuleStatusById, data: { id, checked: event.target.checked } }, () => getData());
  const generateLastMatchedTime = (timestamp) => {
    ...
    <>
      <div className="py-3 px-6 flex justify-between items-center w-full border-b border-slate-700 bg-slate-700 bg-opacity-40">
        ...
        <div className="flex-1">
          Last Matched
          <sup className="text-xs inline-block bottom-4 text-red-500">Beta</sup>
        </div>
        )}
      ...
      <ul className={twMerge('overflow-y-auto min-h-[350px] max-h-[450px]', listClasses)}>
        {data.filter((ruleMetaData) => ruleMetaData.name.includes(search))
          .reverse().map((ruleMetaData) => (
            <li key={ruleMetaData.id} className="py-5 max-h-[90%] flex justify-between items-center px-6 border-b border-slate-700 w-full hover:bg-slate-800 hover:bg-opacity-40">
