...
const generateListeners = (callback) => ({
  ...
  [ListenerType.ON_COMMITTED]: (() => {
    ...
    return {
      addListener: (...args) => chrome.webNavigation.onCommitted.addListener(listener, ...args),
      removeListener: () => chrome.webNavigation.onCommitted.removeListener(listener),
    };
  })(),
