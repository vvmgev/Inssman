...
function createRecorderWidget() {
  ...
  newElement.addEventListener('click', () => {
    ...
    window.postMessage({action: "stopRecording", from: 'content', data: {}}, window.origin);
  });
...
class RecordingService extends BaseService {
  ...
  onUpdateTab = async (tabId: number, _: any, tab: Tab): Promise<void> => {
    ...
    if(tabId === this.currentTab?.id) {
      ...
      InjectCodeService.injectInternalScriptToDocument(tabId, `
        ...
        chrome.runtime.sendMessage({action: ${PostMessageAction.asa}, data: {aaa: 12}});
